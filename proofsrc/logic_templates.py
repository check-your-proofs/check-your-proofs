from ast_types import Term, Var, Con, Compound, Formula, Symbol, Not, And, Or, Implies, Iff, Forall, Exists, ExistsUniq
...
class FormulaTemplate:
    ...
    def apply(self, args: list[Term]) -> "TemplateCall":
        if len(args) != self.arity:
            raise ValueError(f"{self.name} expects {self.arity} args, got {len(args)}")
        return TemplateCall(self, args)
...
def instantiate_templates(formula: Formula, env: dict[FormulaTemplate, Callable[[list[Term]], Formula]]) -> Formula:
...
def make_tree(expr: Formula | Term, prefix: str = "", is_root: bool = True, is_last: bool = True) -> str:
    ...
    if isinstance(expr, Var):
        ...
        return f"{print_prefix}Compound({expr.fun.name}, [{args_str}])"
    elif isinstance(expr, Symbol):
        args_str = ", ".join(make_tree(arg, "", True) for arg in expr.args)
        return f"{print_prefix}Symbol({expr.pred.name}, [{args_str}])"
    elif isinstance(expr, Not):
        ...
        args_str = ", ".join(make_tree(arg, "", True) for arg in expr.args)
        return f"{print_prefix}TemplatesRef({expr.template.name}, [{args_str}])"
    else:
        raise Exception(f"Unexpected expr: {expr}")
...
if __name__ == "__main__":
    ...
    phi = FormulaTemplate("phi", 1)
    phi_z = phi.apply([z])
    separation_schema = Forall(x, Exists(y, Forall(z, Iff(Symbol(pred_in, [z, y]), And(Symbol(pred_in, [z, x]), phi_z)))))
    ...
    instantiated = instantiate_templates(renamed_schema, env)
